    ...
    <?php
        ...
        if(isset($_POST['login'],$_POST['password'])){
            ...
            if($_POST['login'] = ""){ // si le login est vide
                echo "<p type='alert' class='alert alert-danger alert-dismissible fade show'> Le champ nom d'utilisateur est vide.</p>";
            } 
            elseif(mysqli_num_rows($select)) {
                echo "<p type='alert' class='alert alert-danger alert-dismissible fade show'>Ce nom d\'utilisateur existe déjà</p>";
            } 
            elseif($_POST['password']== "" || $_POST['password2']== ""){
                echo "<p type='alert' class='alert alert-danger alert-dismissible fade show'>Le champs Mot de passe est vide.</p>";
            } 
            elseif ($_POST['password'] != $_POST['password2']) { 
                ...
                if(!mysqli_query($conn,"INSERT INTO utilisateurs (login, password) values('".$login."', '".$password."')")) {
                    echo "<p type='alert' class='alert alert-danger alert-dismissible fade show'>Une erreur s'est produite: </p>".mysqli_error($conn);
                } else {
                    echo "<p class='alert alert-success alert-dismissible fade show'>Vous êtes inscrit(e) avec succès! <a href='connexion.php'>Connectez-vous</a></p>";
                    //on n'affiche plus le formulaire
                    $AfficherFormulaire=0;
                    header("refresh:2; url:connexion.php"); // redirection vers la page de connexion
                }
            }
